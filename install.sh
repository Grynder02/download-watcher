...
if [ -d "$HOME/.local/share/applications" ]; then
  sed "s|Exec=.*|Exec=$HOME/.local/bin/download-watcher|" \
...
if [ -d "$HOME/.local/bin" ]; then
  ln -sf "$INSTALL_DIR/watcher/dl-launcher.sh" "$HOME/.local/bin/download-watcher"
  ln -sf "$INSTALL_DIR/watcher/dl-launcher.sh" "$HOME/.local/bin/dl-watcher"
  ln -sf "$INSTALL_DIR/watcher/dl-setup.sh" "$HOME/.local/bin/download-watcher-setup"
...
echo ""
echo "Quick start:"
echo "  1. Run:  download-watcher-setup"
echo "  2. Enter your download details"
echo "  3. Run:  download-watcher"
echo ""
